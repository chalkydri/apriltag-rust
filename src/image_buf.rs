...
impl Image {
    ...
    //     stride: usize,
    // ) -> Result<Self, Error> {
    //     let ptr = unsafe { sys::image_u8_create_stride(width as c_uint, height as c_uint, stride as c_uint) };
    //     let ptr = NonNull::new(ptr).ok_or_else(|| Error::CreateImageError {
    //         reason: "image_u8_create() failed".to_string(),
    //     })?;
    //     Ok(Self { ptr })
    ...
    pub fn into_raw(self) -> sys::image_u8_t {
...
impl AsRef<[u8]> for Image {
    fn as_ref(&self) -> &[u8] {
        unsafe { slice::from_raw_parts(self.ptr.buf, self.stride() * self.height()) }
    }
}

impl AsMut<[u8]> for Image {
    fn as_mut(&mut self) -> &mut [u8] {
        unsafe { slice::from_raw_parts_mut(self.ptr.buf, self.stride() * self.height()) }
    }
}

//impl Clone for Image {
//    fn clone(&self) -> Self {
//        let cloned_ptr = unsafe { sys::image_u8_copy(self.ptr) };
//        Self {
//            ptr: unsafe { cloned_ptr.read() },
//        }
//    }
//}
...
//    fn drop(&mut self) {
//        unsafe {
//            sys::image_u8_destroy(&mut self.ptr);
//        }
//    }
//}
